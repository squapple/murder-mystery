...
// Phase 39에서 삭제한 게 실수였다는 지적을 받아 복원했다. (2) 김영훈 휴대폰 통화·문자
// 기록 물증을 신설해 박서연·이현우 둘 다에게 사망 직전 접촉 흔적을 남겼다 — 초반엔
// 둘 다 의심하게 만드는 장치. (3) 정민아의 "법인카드 비리 목격" 서브플롯은 완전히
// 삭제하고, 그날 밤 심란해 보인 이유를 "김영훈과의 로비 다툼"(다른 팀원 목격) 하나로
// 통합했다.
//
// Phase 41 — 사용자가 확정한 3가지: (1) 이현우는 계획적으로 살해했다(몸싸움 없이) —
// 다만 부검 소견 등 플레이어 공개 텍스트에는 "계획적"/"우발적" 어느 쪽도 명시하지
// 않는다. (2) CCTV 관련 증거를 전부 삭제하고, "이현우가 CCTV가 드문 곳을 워크숍
// 장소로 직접 제안했다"는 설정으로 대체했다(stmt-lee-workshop-planner에 반영).
// (3) 그날 밤(20:00~24:00) 비가 내렸다는 설정을 추가해, 신발 증거의 근거를
// "이슬/습기"에서 "빗물"로 바꿨다 — 박서연이 신발을 씻어낸 이유도 "발 씻는 습관 +
// 다른 팀원 목격담"이라는 다소 작위적인 설정 대신 "비에 젖어서 슬리퍼를 대충
// 헹궈냈다"는 평범한 행동으로 단순화됐다(`stmt-park-shoe-washing` 삭제).
//
...
export const EVIDENCE: EvidenceItem[] = [
  {
    ...
    // Phase 41: "몸싸움 중 우발적으로 벌어진 상해" 표현을 삭제했다 — 실제로는 몸싸움
    // 없이 벌어진 계획적 범행이지만, 그 사실을 부검 소견에 직접 명시하지 않는다
    // (사용자 지시: "계획적이라고 언급하지는 말자"). 물리적 소견만 서술한다.
    detail:
      "부검 소견: 어깨와 옆구리 부위에 예리한 흉기에 의한 자상 다수 확인. 직접 사인은 과다출혈. 위 내용물 소화 정도를 근거로 사망 추정 시각은 21:45경으로 특정됐다. 흉기 자체는 현장에서 발견되지 않았다.",
  ...
  },
  {
    ...
    category: "physical",
    name: "편의점 영수증",
    // Phase 41: 박서연의 편의점 외출이 21:05(문자를 보낸 직후)로 당겨졌다 — 22:15에
    // 회식으로 복귀하기 전까지 정확히 어디 있었는지는 이 영수증 하나로 전부 증명되지
    // 않는다(사망 추정 시각 21:45와 가장 가까운 시간대라 오히려 더 미묘하다).
  ...
  },
  {
    ...
    // Phase 41: CCTV 관련 증거를 전부 삭제하는 대신, "이현우가 CCTV가 거의 없는
    // 외진 곳을 직접 제안했다"는 장소 특징을 이 카드에 얹었다 — 해석("그래서
    // 일부러?")은 붙이지 않고 사실만 나열한다.
    detail:
      "이번 워크숍 장소 예약과 숙소 배정을 이현우가 직접 맡아 진행했다는 이야기가 있다 — 이 연수원은 산책로 등 일부 구역에 CCTV가 거의 설치되어 있지 않은, 비교적 외진 곳이다.",
    revealTiming: "round2_end",
  },
  {
  ...
  {
    // Phase 41 — 신발 증거의 근거를 "이슬/습기"에서 "빗물"로 교체했다(그날 밤
    // 20:00~24:00 비가 내렸다는 설정 추가). 흙 성분 일치(산책로에 있었다) + 빗물에
    // 젖은 흔적(비가 오던 시간대, 즉 밤에 갔다는 뜻) 두 가지를 한 번의 신발 확인으로
    // 함께 제시한다. "몇 시인지/얼마나 있었는지/어디까지 들어갔는지"는 특정되지
    // 않는다 — "그날 밤 산책로에 있었다"는 강한 심증은 서지만 "죽였다"까지는
    // 확정되지 않는 선에서 멈춰야, 박서연 방에서 나온 흉기와 논리적으로 충돌하며
    // ("거짓말한 건 이현우인데 흉기는 왜 박서연 방에?") 플레이어의 최종 판단을 계속
    // 흔들어놓는 핵심 장치가 유지된다(사용자 지시). 이현우의 2단계 방어(낮 프로그램
    // 핑계 → 빗물 증거 제시되면 "초입까지만, 못 만났다"로 한 단계만 후퇴하고 그
    // 이상은 절대 안 무너짐)는 characters.ts knownSecrets에 서술해 액터가 한 응답
    // 안에서 그 단계적 반응을 연기하도록 한다.
    id: "ev-shoe-lee",
    category: "physical",
    name: "이현우 신발 확인",
  ...
  {
    // Phase 41: 박서연의 신발 설명을 단순화했다 — "발 씻는 습관 + 다른 팀원 목격담"
    // (Phase 40, 다소 작위적)을 없애고, 그냥 "비에 젖어서 슬리퍼를 헹궈냈다"는
    // 평범한 행동으로 바꿨다(사용자 지시 — 마침 슬리퍼였던 것도 자연스러운 이유).
    id: "ev-shoe-park",
    category: "physical",
    name: "박서연 신발 확인",
    revealedFact: "박서연 신발 확인 — 산책로 흔적 없음",
    detail:
      "낮 산책로 프로그램에 참가했음에도 신발에 산책로 흔적이 전혀 없다 — 그날 밤 비가 와서 슬리퍼에 흙탕물이 좀 튀었었는데, 물로 대충 헹궈냈을 뿐이라고 설명한다. 사건과는 무관해 보인다.",
    revealTiming: "action_triggered",
  },
  {
