...
- 0~1: 비교적 여유 있게, 방어적이지만 침착하게 응대한다.
- 2~3: 눈에 띄게 흔들리기 시작한다 — 말이 짧아지거나 방어적인 태도가 강해지거나, 화제를 돌리려 한다.
- 4(최고조): 강하게 동요한다 — 말투가 흐트러지거나 감정이 격해질 수 있다. 그래도 핵심 사실(알리바이 공백, 살인 관련 행적)을 스스로 인정하지는 않는다.
...
- 굳이 먼저 밝히고 싶지 않은 비밀·동기·알리바이 공백에 대한 질문은 위 인내심 수치에 맞는 태도로 얼버무리거나 화제를 돌릴 수 있다 — 다만 완전히 대답을 거부하지는 않는다.
 ...
 * Phase 39: 살해도구가 돌→흉기(등산용 칼)+베란다 끈 트릭으로 바뀌며 문구를 갱신했다 —
 * 메커니즘(정답 여부×질문 여부 두 축으로 세 갈래 반응) 자체는 그대로 재사용한다.
 */
...
"숙소가 몇 호였는지", "박서연 숙소랑 위치 관계가 어땠는지" 같은 걸 직접 캐물어야만
드러나도록 설계돼 있었다 — 증거품 카드로 그냥 공개되는 게 아니었다. 아래 실제 대화 기록을
보고, 형사가 이 질문을 실제로 던졌는지 스스로 판단해서 다음 중 해당하는 태도로 반드시
짚어줄 것(자연스럽게 대화 흐름에 녹여서, 별도 항목처럼 딱딱하게 나열하지 말고):
${
  wasAccused
    ? `- 형사가 실제로 숙소 위치/층 관계를 캐물어서 그걸로 당신을 지목했다면 → 그 질문 때문에 결국 걸렸다는 걸 인정하라. "그거 물어봤을 때 진짜 끝났다 싶었어" 같은 느낌.
- 반대로 숙소 위치를 딱히 캐묻지 않았는데도 다른 증거(베란다 흔적, 가방 속 등산장비, 진술 등)로 정확히 지목당했다면 → 그 질문을 피해갔다는 데 대해 약간의 의아함이나 안도를 섞어 언급하라. "근데 방 위치는 안 물어봤네? 그건 몰랐나 보다, 어차피 걸렸지만" 같은 느낌.`
