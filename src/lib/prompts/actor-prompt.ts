...
//    자백했는지만 보면 추리 없이 정답을 알 수 있었기 때문. 실제 자백(과 무고자의
//    진실 공개)은 최종 지목 이후 결과 화면에서 buildDebriefSystemPrompt로 별도
//    생성한다(Phase 27, "AI 친구" 디브리핑 — 이력 하단 참고).
...
//    주석 참고.
// 10) "AI 친구" 컨셉 도입(Phase 27) — 이 게임은 여러 성향의 AI 중 3명을 뽑아 배역을
//     맡기고 함께 파티 게임을 하는 컨셉이다. 원래는 진범만 결과 화면에서 "자백"
//     장면을 연기했고 무고자는 motiveFull 한 줄만 노출돼, 무고자의 곁가지 비밀
//     (예: 정민아의 법인카드 비리 목격)이 결과 화면에 전혀 드러나지 않는 문제가
//     있었다. 캐릭터 연기를 내려놓고 "이 역할을 맡았던 AI 친구"(페르소나별로
//     이름을 붙였다 — personas.ts의 friendName)로 돌아와 뒤풀이 소감을 말하는
//     디브리핑으로 3배역 전부 통일했다(buildDebriefSystemPrompt). 진범이든
//     무고자든 "나는 이런 역할이었고 심문 중 이랬다"는 구조는 동일해 프롬프트도
//     하나면 충분했고, 부수적으로 정민아 서브플롯 문제도 함께 해결됐다.
 ...
 * GameApp.tsx가 인내심 표시 배지를 그리기 위해 그대로 가져다 쓰므로 export한다
 * (Phase 30 — 사용자 피드백: 인내심 시스템이 있다는 걸 플레이어가 알 방법이 없었다).
 */
export const PRESSURE_LABEL = "동요";
 ...
 * (사용자 지침 — 답을 그냥 공개하는 카드로 만들지 않는다). 그래서 결과 화면에서
 * 이현우의 AI 친구가 "너 이걸 물어봤는지/안 물어봤는지"를 짚어주는 게 사실상 유일한
 * 사후 힌트 — 플레이어가 못 찾은 경로를 자연스럽게 알려주는 장치다. 정답 여부(범인
 * 지목 성공)와 방 위치 질문 여부, 두 축으로 세 갈래 반응이 갈린다:
 *   (a) 지목 성공 + 대화에서 방 위치를 캐물었음 → 그 경로로 잡혔다고 인정
 *   (b) 지목 성공 + 방 위치는 안 물어봤음 → 다른 경로로 잡힌 것에 안도/의아함
 *   (c) 지목 실패 → 방 위치를 안 물어봐서 살았다는 안도(살짝 여유 있는 태도)
 * 셋 다 아래 실제 대화 기록을 직접 보고 모델이 스스로 (a)/(b)를 판단하게 하며,
 * 정답 여부는 wasAccused로 이미 확정돼 있다(이현우가 실제 진범이므로 그를 지목했다면
 * 곧 정답이다).
 */
...
베란다를 통해 돌을 아래층으로 옮길 수 있었다는 사실은, 형사가 심문 중 "숙소가 몇 호였는지",
"박서연 숙소랑 위치 관계가 어땠는지" 같은 걸 직접 캐물어야만 드러나도록 설계돼 있었다 —
증거품 카드로 그냥 공개되는 게 아니었다. 아래 실제 대화 기록을 보고, 형사가 이 질문을
실제로 던졌는지 스스로 판단해서 다음 중 해당하는 태도로 반드시 짚어줄 것(자연스럽게
대화 흐름에 녹여서, 별도 항목처럼 딱딱하게 나열하지 말고):
${
  wasAccused
    ? `- 형사가 실제로 숙소 위치/층 관계를 캐물어서 그걸로 당신을 지목했다면 → 그 질문 때문에 결국 걸렸다는 걸 인정하라. "그거 물어봤을 때 진짜 끝났다 싶었어" 같은 느낌.
- 반대로 숙소 위치를 딱히 캐묻지 않았는데도 다른 증거(신발흙, CCTV, 진술 등)로 정확히 지목당했다면 → 그 질문을 피해갔다는 데 대해 약간의 의아함이나 안도를 섞어 언급하라. "근데 방 위치는 안 물어봤네? 그건 몰랐나 보다, 어차피 걸렸지만" 같은 느낌.`
    : `- 형사가 숙소 위치/층 관계를 캐묻지 않아서(혹은 캐물었어도 다른 사람을 지목해서) 결국 잡히지 않았다면 → 은근히 안도하거나 여유 부리는 투로, 그 질문을 했다면 빠져나갈 구멍이 없었을 거라는 뉘앙스를 흘려라. "사실 방 위치까지 물어봤으면 나 진짜 답 없었는데... 다행이다" 같은 느낌.`
}`;
}
 ...
 * 한 줄만 보여줬는데(그래서 정민아의 법인카드 비리 같은 곁가지 비밀이 결과 화면에
 * 전혀 안 드러나는 문제가 있었다), 사용자 제안으로 셋 다 동일한 디브리핑으로
 * 통합했다 — 진범이든 무고자든 "나는 이런 역할이었고, 이런 성향으로 임했고,
 * 심문 중 이런 점이 힘들었다"는 구조는 똑같으므로 프롬프트도 하나로 충분하다.
 * 이전 자백 전용 프롬프트와 같은 이유로 심문용 buildActorSystemPrompt를 재사용하지
 * 않는다 — 심문 중 규칙(부인 유지 등)이 전혀 없는 완전히 새 프롬프트라 애초에
 * 규칙 충돌이 생길 수 없다.
 */
