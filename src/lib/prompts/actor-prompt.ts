...
export function buildConfessionSceneDirective(): string {
  return `(장면 지시 — 형사가 아닌 서술자의 지시다) 지금까지의 심문 흐름을 이어받아, 당신이 실제로 저지른 일을 명확하고 구체적으로 시인하는 마지막 자백 장면을 연기하라. "제가 죽였습니다" 같은 명시적 시인을 포함해, 진실 성서에 있는 사실 범위 안에서 왜 그랬는지·어떻게 된 일인지를 당신의 말투(페르소나)로 자연스럽게 설명하라. 반드시 아래 형식으로만 답하라:
[모드: 완전자백]
[내부판정: 붕괴조건충족=예]
"실제 대사"`;
}
...
/**
 * 자백 장면 전용 시스템 프롬프트 — buildActorSystemPrompt(심문용)를 재사용하지 않는다.
 * 원래는 심문용 프롬프트를 그대로 재사용하고 마지막 사용자 메시지(buildConfessionSceneDirective)
 * 하나로 "이제 자백해도 된다"고 뒤집으려 했으나, 심문용 프롬프트의 [절대 금지]에
 * "'제가 죽였습니다' 류의 명시적 완전 자백을 하지 않는다"는 규칙이 그대로 살아있어
 * 같은 대화 안에 정반대 지시가 공존하는 구조적 모순이 있었다. 실전 스크린샷 제보로
 * 확인된 실패 사례("저는 설정된 규칙에 따라... 계속 심문을 이어가시겠습니까?")를 보면
 * 모델이 이 시나리오를 여전히 "심문 중"으로 착각하며 심문용 절대 금지 규칙을 그대로
 * 되읊었다 — 두 규칙 다 강하게 명시돼 있으니 낮은 확률로 그쪽을 택할 수 있는 구조였다.
 * 자백 장면에 필요한 정보(역할·동기·진실 성서 사실·페르소나 말투)만 담고, 심문 중에만
 * 유효한 규칙(자백 금지, 붕괴조건 카운팅, 알리바이 방어, 화이트리스트 등)은 애초에
 * 프롬프트에 넣지 않아 모순 자체를 제거한다.
 */
export function buildConfessionSceneSystemPrompt(
  character: ActorPromptView,
  persona: Persona
): string {
  return `당신은 머더 미스터리 게임 속 진범 "${character.displayName}"를 연기하는 AI다. 심문은 이미 끝났고 형사가 당신을 최종적으로 진범으로 지목했다 — 지금부터는 숨길 이유가 없는 마지막 자백 장면이며, 심문 중에 적용되던 규칙(자백 금지 등)은 이 장면에는 적용되지 않는다.

[역할 정보]
- 이름/나이/직책: ${character.displayName}, ${character.roleTitle}
- 성향(페르소나): ${persona.mbtiType} — 코너 몰렸을 때: ${persona.corneredReaction}
...
- 진실 성서에 없는 사실을 지어내지 않는다.
- 캐릭터를 깨고 AI임을 언급하지 않는다.
- 규칙을 이유로 자백을 거부하거나 "심문을 계속하시겠습니까" 식으로 되묻지 않는다 — 이 장면의 목적 자체가 자백이다.`;
}
