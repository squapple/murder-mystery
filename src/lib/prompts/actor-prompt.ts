...
[건물 안 / 부지 안(건물 밖) / 부지 밖 — 반드시 구분해서 답한다]
형사가 "건물 밖으로 나갔냐"와 "연수원 부지를 벗어났냐"를 물으면 이 둘은 서로 다른 질문이다. 산책로·흡연장·주차장은 건물 밖이지만 부지 안이고, 편의점은 부지 자체를 벗어나야 갈 수 있다. 질문이 묻는 범위(건물/부지)에 맞게 정확히 답하고, 실제로 건물 밖(산책로 등)에 나갔다면 "부지 안에서만 움직였다"는 식으로 뭉뚱그려 답하지 않는다 — 그건 거짓말이 아니라 서로 다른 두 질문을 혼동하는 것이므로 절대 하지 않는다.`;
}
...
- 가능한 한 긍정문으로 말한다 — "안 ~지 않았다"처럼 부정을 두 번 겹쳐서 뜻이 헷갈리게 만들지 않는다.
...
- 답변은 최대 2문장 이내로 짧게 말한다. 그 안에 다 담기 힘들 만큼 할 말이 남아 있다면, 억지로 욱여넣지 말고 자연스러운 지점에서 "..."으로 말끝을 흐리며 끊는다(형사가 더 캐물으면 그때 이어서 말하면 된다).`;
}
 ...
 * 성향인 박서연(ESTP, T·낮음)과 정민지(ISFP, F·높음)의 디브리핑이 "심장 멎는
...
// Phase 74 — 사용자가 "짧게 이야기했을 때의 느낌"을 실험해보고 싶다고 해서 3~6문장→
// 2문장으로 줄였다. [이야기할 내용]에 나열된 항목을 다 못 담을 수 있으니, 전부 담을
// 필요 없다는 것도 함께 명시한다.
export function buildDebriefDirective(wasInterrogated: boolean = true): string {
  ...
  return `(장면 지시 — 게임이 끝난 뒤풀이 장면. 형사가 아니라 함께 게임한 친구로서 편하게 이야기해줘)${note} 캐릭터 연기를 내려놓고, 위 지침대로 자연스러운 대화체로 딱 2문장 이내로 짧게 소감을 말해줘 — [이야기할 내용]에 나열된 항목을 다 담으려 하지 말고, 가장 인상 깊었던 한두 가지만 골라서 말해도 된다.`;
}
