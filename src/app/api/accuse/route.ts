...
// 않는다 (actor-prompt.ts buildConfessionSceneDirective 주석 참고: 심문 중 자백을
// 허용하면 "누가 자백했는지"만 보고 추리 없이 정답을 알 수 있어 게임이 무의미해진다).
  ...
  try {
    ...
    if (culprit && culpritPersona) {
      ...
      const messages: OpenAI.Chat.ChatCompletionMessageParam[] = [
        { role: "system", content: buildActorSystemPrompt(getActorPromptView(culprit), culpritPersona) },
        ...culpritHistory.map(
